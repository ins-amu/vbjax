...
def init_ct_mhsa(hp: Hyperparameters, key, batch_size: int = 1, initial_c: Optional[jax.Array] = None, lengths: Optional[jax.Array] = None) -> Tuple[CTMHSAParams, NetworkState, Optional[jax.Array]]:
    ...
    params = CTMHSAParams(W_Q=W_Q, W_K=W_K, W_V=W_V, W_Y=W_Y, C=C, ln_gamma=ln_gamma, ln_beta=ln_beta)
...
def network_coupling(y_t: jax.Array, params: CTMHSAParams, external_input: jax.Array, 
                     history: Optional[jax.Array] = None, step: int = 0, lags: Optional[jax.Array] = None) -> jax.Array:
    """
...
def scan_ct_mhsa(params: CTMHSAParams, init_state: NetworkState, inputs: jax.Array, hp: Hyperparameters, lags: Optional[jax.Array] = None) -> Tuple[Tuple[NetworkState, jax.Array], jax.Array]:
    ...
    def step_fn(carry, input_t):
        ...
        def micro_step_fn(i, inner_carry):
            ...
            return (state_new, y_new)



        # Run micro-steps

        final_inner_carry = jax.lax.fori_loop(0, hp.steps_per_token, micro_step_fn, carry)

        

        # Return result of last micro step as the output for this sequence step

        return final_inner_carry, final_inner_carry[1]
