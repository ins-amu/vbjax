...
JRState = collections.namedtuple(
    typename="JRState", field_names="y0 y1 y2 y3 y4 y5".split(" ")
)
...
BVEPTheta = collections.namedtuple(typename="BVEPTheta", field_names="tau0 I1 x0")

bvep_default_theta = BVEPTheta(tau0=10.0, I1=3.1, x0=-3.5)
...
MPRTheta = collections.namedtuple(
    typename="MPRTheta", field_names="tau I Delta J eta cr cv".split(" ")
)

mpr_default_theta = MPRTheta(
    tau=1.0, I=0.0, Delta=1.0, J=15.0, eta=-5.0, cr=1.0, cv=0.0
)

MPRState = collections.namedtuple(typename="MPRState", field_names="r V".split(" "))
...
def bold_dfun(sfvq, x, p: BOLDTheta):
    ...
    dq = p.recip_tau_o * (
        f * (1 - (1 - p.e0) ** (1 / f)) * p.recip_e0 - v**p.recip_alpha * (q / v)
    )
    return np.array([ds, df, dv, dq])


DCMTheta = collections.namedtuple(typename="DCMTheta", field_names="A,B,C")
...
DopaState = collections.namedtuple(typename="DopaState", field_names="r V u Sa Sg Dp")
...
dopa_default_initial_state = DopaState(r=0.03, V=-67.0, u=0.0, Sa=0.0, Sg=0.0, Dp=0.5)
...
def dopa_dfun(y, cy, p: DopaTheta):
    ...
    dr = 2.0 * a * r * V + b * r - ga * Sa * r - gg * Sg * r + (a * Delta) / np.pi
    dV = (
        a * V**2
        + b * V
        + c
