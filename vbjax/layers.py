...
def create_degrees(key, n_inputs, n_hiddens, input_order, mode):
    ...
    if isinstance(input_order, str):
        ...
        assert jnp.all(jnp.sort(input_order) == jnp.arange(1, n_inputs + 1)), (
            "invalid input order"
        )
    ...
    if mode == "random":
        for N in n_hiddens:
            ...
            degrees_l = jax.random.randint(
                key, shape=(N,), minval=min_prev_degree, maxval=n_inputs
            )
...
if _HAS_FLAX:
    ...
    class MaskedLayer(nn.Module):
        ...
        kernel_init: Callable = (
            lambda key, shape, mask: random.normal(key, shape=shape) * 1e-3 * mask
        )
    ...
    class OutputLayer(nn.Module):
        ...
        out_mask: ArrayImpl
        kernel_init: Callable = (
            lambda key, shape, out_mask: jax.random.normal(key, shape=shape)
            / jnp.sqrt(shape[0])
            * out_mask
        )
        bias_init: Callable = nn.initializers.zeros_init()
        ...
        def __call__(self, inputs):
            kernel_m = self.param(
                "kernel_m",
                self.kernel_init,
                (inputs.shape[-1], self.out_dim),
                self.out_mask,
            )
            kernel_logp = self.param(
                "kernel_logp",
                self.kernel_init,
                (inputs.shape[-1], self.out_dim),
                self.out_mask,
    ...
    class FourierLayer(nn.Module):
        kernel_init: Callable = lambda key, shape: jax.random.normal(
            key, shape=shape
        ) / jnp.sqrt(shape[0])
        bias_init: Callable = nn.initializers.zeros_init()
