...
"""Vendor of jax.example_libraries.optimizers (deprecated upstream).

This module will be replaced by Optax in a future vbjax release.
...
# Inlined private utils from jax._src.util
...
def safe_zip(*args):
    assert all(_builtin_len(a) == _builtin_len(args[0]) for a in args)
    return _builtin_list(_builtin_zip(*args))

def safe_map(f, *args):
    return _builtin_list(_builtin_map(f, *args))

def unzip2(xs):
    return _builtin_tuple(_builtin_zip(*xs)) if xs else ((), ())
